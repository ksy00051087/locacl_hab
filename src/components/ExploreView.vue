          ...
          <div v-for="place in tour.filteredPlaces.value" :key="place.contentid" class="bg-white rounded-2xl p-4 border border-slate-200 hover:border-primary shadow-xs transition-all flex flex-col group relative overflow-hidden">
            <div class="flex space-x-4 items-center">
              ...
              <div class="flex-grow min-w-0">
                ...
                <button @click="tour.togglePlaceExpansion(place.contentid, chat.handleAdvisorAutoTrigger)" class="text-xs text-accent hover:text-accenthover font-bold mt-1.5 flex items-center gap-1">
                  <span>{{ tour.state.expandedPlaceId === place.contentid ? '상세 설명 닫기' : '상세 설명 및 지도 보기' }}</span>
