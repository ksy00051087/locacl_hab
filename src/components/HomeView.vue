<template>
  <div class="space-y-6 animate-fade-in">
    <div class="bg-primary/20 text-slate-800 p-5 rounded-3xl shadow-xs border border-primary/60 space-y-2.5">
      ...
      <p class="text-xs md:text-sm text-slate-600 leading-loose font-semibold">
    ...
    <div class="bg-white p-5 lg:p-7 rounded-3xl shadow-xs border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6 relative">
      <div class="space-y-3.5 z-10 max-w-2xl">
        ...
        <h2 class="text-3xl lg:text-4xl font-black text-slate-800 leading-normal">구미/경북의 새로운 여행 파트너</h2>
        <p class="text-slate-600 text-sm leading-8 font-medium">
          <span class="text-slate-700 font-bold text-xl">"새로운 로컬 인연과의 만남"</span><br />
      ...
      <div class="w-full md:w-80 bg-slate-50 p-4 rounded-2xl border border-slate-200 z-10 space-y-2.5 relative">
        <label class="block text-base font-semibold text-slate-700 uppercase tracking-wider"><i class="fa-solid fa-magnifying-glass text-accent"></i> 통합 명소 검색</label>
        <div class="relative">
          <input
            type="text"
            ...
            class="w-full bg-white text-slate-800 rounded-xl pl-3 pr-10 py-2.5 text-xs outline-none border border-slate-200 focus:border-primary focus:ring-2 focus:ring-emerald-50 transition-all font-semibold"
          />
        ...
        <p class="text-xs text-slate-400 font-medium pl-2">금오산, 금리단길, 화본역, 에코랜드 등 입력 후 엔터!</p>
      </div>
    </div>
        ...
        <div class="infinite-track" :style="{ transform: `translateX(-${tour.state.trackOffset}px)` }">
          ...
          >
            ...
            <div class="p-3.5 bg-white border-t border-slate-100">
              <h4 class="font-black text-base text-slate-800 line-clamp-1 group-hover:text-accent transition-colors">{{ spot.title }}</h4>
      ...
      <div class="lg:col-span-4 bg-white rounded-3xl p-5 shadow-xs border border-slate-200 flex flex-col justify-between">
        <div class="space-y-4">
          <div class="flex items-center justify-between border-b border-slate-100 pb-2.5">
            <div>
              <h3 class="font-extrabold text-lg text-slate-800 flex items-center gap-1.5"><i class="fa-solid fa-people-group text-accent"></i> 같이 여행 일정</h3>
              ...
              <div class="flex items-start space-x-2.5 min-w-0">
                ...
                <div class="min-w-0">
                  <div class="flex items-center gap-1.5 mb-0.5 flex-wrap">
                    <span class="bg-primary text-slate-900 text-[10px] font-semibold px-1.5 py-0.5 rounded-md uppercase">{{ community.getPostCategoryDetails(post).name }}</span>
                    <span class="text-xs text-accent font-semibold"><i class="fa-regular fa-calendar"></i> {{ post.travelDate }}</span>
                  </div>
