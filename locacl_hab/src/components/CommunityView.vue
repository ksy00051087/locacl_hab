<template>
  <div class="space-y-6 animate-fade-in">
    <div class="bg-white rounded-3xl p-5 shadow-xs border border-slate-200">
      <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-5 space-y-1.5 text-base">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div class="flex space-x-1.5 overflow-x-auto pb-1">
          <button @click="community.state.communityTypeFilter = 'all'" class="text-sm font-semibold px-2.5 py-1.5 rounded-xl border shrink-0 transition-all" :class="community.state.communityTypeFilter === 'all' ? 'bg-primary border-primary text-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'">전체보기</button>
          <button @click="community.state.communityTypeFilter = 'general'" class="text-sm font-semibold px-2.5 py-1.5 rounded-xl border shrink-0 transition-all" :class="community.state.communityTypeFilter === 'general' ? 'bg-accent border-accent text-white' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'">일반 자유글</button>
          <button @click="community.state.communityTypeFilter = 'recruitment'" class="text-sm font-semibold px-2.5 py-1.5 rounded-xl border shrink-0 transition-all" :class="community.state.communityTypeFilter === 'recruitment' ? 'bg-primary border-primary text-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'">동행 모집글</button>
        </div>
        <div class="flex items-center space-x-2 text-sm">
          <span class="text-slate-400 font-medium shrink-0">지역구 필터:</span>
          <select v-model="community.state.selectedRegionFilter" class="bg-white border border-slate-200 rounded-xl px-2.5 py-1.5 outline-none text-sm font-medium text-slate-700">
          ...
          <div v-show="post.isOpen" class="border-t border-slate-200 p-4 bg-slate-50/50 rounded-b-2xl animate-fade-in space-y-3.5">
            <div v-if="post.type === 'recruitment'" class="bg-white p-3.5 rounded-xl border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <button
                ...
                class="text-sm font-medium px-3 py-2 rounded-xl transition-all shadow-xs shrink-0"
              ...
              >
                ...
                <span v-else><i class="fa-solid fa-user-plus text-[12px]"></i> 메이트 참여 ({{ post.currentCount }}/{{ post.maxCount }})</span>
              </button>
            </div>

            <div class="text-base md:text-base text-slate-700 font-medium whitespace-pre-wrap leading-relaxed">{{ post.content }}</div>
            ...
            <div class="flex items-center justify-between text-sm text-slate-400 border-t border-slate-200 pt-3 flex-wrap gap-2">
              <div class="flex items-center space-x-2">
                <span class="font-extrabold text-slate-500"><i class="fa-solid fa-user-secret text-accent"></i> 익명메이트</span>
                <span>•</span>
                <span class="font-medium">{{ post.createdDate }}</span>
              </div>
              <div class="flex items-center space-x-3">
                <button @click="community.togglePostLike(post)" class="hover:text-red-500 flex items-center gap-0.5 font-medium">
                  <i class="fa-solid fa-heart" :class="{ 'text-red-500': post.liked }"></i> {{ post.likes }}
                </button>
                <span>•</span>
                <button @click="community.initiateEdit(post)" class="text-slate-400 hover:text-accent font-medium">수정</button>
                <button @click="community.initiateDelete(post)" class="text-slate-400 hover:text-red-600 font-medium">삭제</button>
              </div>
            </div>

            <div class="border-t border-slate-200 pt-3 space-y-2.5">
              <h5 class="text-sm font-medium text-slate-700 flex items-center gap-1.5"><i class="fa-regular fa-comments"></i> 익명 실시간 댓글 ({{ post.comments ? post.comments.length : 0 }})</h5>
              ...
              <div v-if="post.comments && post.comments.length > 0" class="space-y-2 bg-white p-3 rounded-2xl border border-slate-200 max-h-48 overflow-y-auto">
                <div v-for="(comment, cIdx) in post.comments" :key="cIdx" class="text-sm leading-relaxed border-b border-slate-100 pb-1.5 last:border-b-0 last:pb-0">
                  <div class="flex justify-between items-center text-[12px] text-slate-400 font-medium mb-0.5">
                    <span class="text-accent font-medium"><i class="fa-solid fa-reply"></i> 익명 여행가</span>
                    <span>{{ comment.date }}</span>
                  </div>
                  <p class="text-slate-700 font-medium text-sm">{{ comment.content }}</p>
                </div>
              </div>
              <div v-else class="text-[12px] text-slate-400 font-medium py-1">작성된 댓글이 없습니다. 첫 익명 소통 한마디를 나누어보세요!</div>
              ...
              <div class="flex gap-2">
                <input
                  ...
                  class="flex-grow bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-primary font-medium"
                />
                <button @click="community.submitComment(post)" class="bg-accent hover:bg-accenthover text-white text-sm font-medium px-3 rounded-xl transition-colors shrink-0">등록</button>
              </div>
            </div>
          </div>
        </div>

        <div v-if="community.sortedPosts.value.length === 0" class="py-12 text-center text-slate-400 text-sm bg-white rounded-3xl border border-dashed border-slate-200">
